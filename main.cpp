...
void solve()
{
    ...
    m[{'A','A'}]=1;  m[{'H','H'}]=1;  m[{'I','I'}]=1;  m[{'M','M'}]=1;  m[{'O','O'}]=1;  m[{'T','T'}]=1;  m[{'U','U'}]=1;  m[{'V','V'}]=1;  m[{'W','W'}]=1;  m[{'X','X'}]=1;  m[{'b','d'}]=1;  m[{'o','o'}]=1;  m[{'p','q'}]=1;  m[{'v','v'}]=1;  m[{'w','w'}]=1;  m[{'x','x'}]=1; m[{'Y','Y'}]=1;
    string s;
    cin>>s;
    f(i,0,(s.size()+1)/2){
        if(m[{s[i],s[s.size()-i-1]}]==0 && m[{s[s.size()-i-1],s[i]}]==0){
            cout<<"NIE"<<endl; rtn;
        }
    }
        cout<<"TAK"<<endl;
